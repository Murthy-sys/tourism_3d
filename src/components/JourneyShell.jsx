...
export default function JourneyShell(){
  ...
  const goTo=id=>{const selected=CHAPTERS.find(item=>item.id===id),start=getProgressForChapter(id),p=selected?(start+selected.progressEnd)/2:start,el=track.current;if(!el)return;const travel=el.offsetHeight-innerHeight;scrollTo({top:el.offsetTop+p*travel,behavior:reducedMotion||innerWidth<768?'auto':'smooth'})}

...
import { setAmbientEnvironment } from '../audio/ambient'

export default function JourneyShell({soundOn=false}){
  ...
  useEffect(()=>{setAmbientEnvironment(progress<.41?'journey':progress<.62?'jungle':progress<.75?'water':'mountain')},[progress])
  ...
  return <main className="experience"><section className="experience__track" ref={track}><div className="experience__stage"><div className="experience__sky"/><Hero3D progress={progress} reducedMotion={reducedMotion} onFallback={()=>setFallback(true)}/><div className="experience__grade"/>
    ...
    <div className="edge-controls"><JourneyMenu open={menuOpen} onOpen={()=>setMenuOpen(true)} onClose={()=>setMenuOpen(false)} onSelect={goTo} onBook={()=>book()}/><SoundToggle initialOn={soundOn}/></div>

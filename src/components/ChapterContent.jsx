...
export default function ChapterContent({chapter,progress,onPlan,onBook}){
  const local=Math.min(1,Math.max(0,(progress-chapter.progressStart)/(chapter.progressEnd-chapter.progressStart)))
  ...
  return <article className={`chapter chapter--${chapter.layout}`} key={chapter.id} aria-labelledby={`chapter-${chapter.id}`}>
    ...
    {chapter.layout==='monument-plans'&&<div className="monument-plan-actions">{TRAVEL_PLANS.map((p,i)=><button className={Math.min(2,Math.floor(local*3))===i?'active':''} aria-label={`Plan ${p.name} journey`} key={p.id} onClick={()=>onPlan(p)}><span>0{i+1} · {p.style}</span><strong>{p.name}</strong><small>{p.days} · {p.route}</small><b>Plan this journey ↗</b></button>)}</div>}
